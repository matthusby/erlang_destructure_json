...
parse(Input) when is_binary(Input) ->
	setup_memo(),
	Result = case 'object'(Input,{{line,1},{column,1}}) of
		{AST, <<>>, _Index} -> AST;
		Any -> Any
	end,
	release_memo(), Result.
...
'object'(Input, Index) ->
	p(Input, Index, 'object', fun(I,D) -> (p_seq([fun 'var'/2, fun 'path'/2]))(I,D) end, fun(Node, Idx) -> 
[_Var, PathFun] = Node,
fun(JSON) ->
	PathFun(JSON)
end
...
'int'(Input, Index) ->
	p(Input, Index, 'int', fun(I,D) -> (p_one_or_more(p_charclass(<<"[0-9]">>)))(I,D) end, fun(Node, Idx) -> binary_to_number(Node) end).

'var'(Input, Index) ->
	p(Input, Index, 'var', fun(I,D) -> (p_seq([p_charclass(<<"[_a-zA-Z]">>), p_zero_or_more(p_charclass(<<"[_a-zA-Z0-9]">>))]))(I,D) end, fun(Node, Idx) -> list_to_binary(Node) end).
...
binary_to_number([B]) ->
	list_to_number(erlang:binary_to_list(B)).
